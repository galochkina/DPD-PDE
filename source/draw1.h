...
private:
	...
	{
		MyDc->SaveDC();
		VecF2 dR = (p.isMP()) ? stp->atom.r * VecF21 : stp->atom.rShow * VecF21;
		...
		if(p.isPlasma())
		{
			...
			//if(p.prep) c = RGB(100, 50, 50);
			//else c = RGB(100, 50, 50);
		}
		...
		else if(p.isSPlt())
		{
			if(drPlU)
			{
				if(p.oldPlt) c = RGB(100,100,0);
				else c = RGB(100,200,0);
			}
			else
				c = RGB(0,0,0);
		}
	...
	{	
		...
		if(text)
		{
			...
			{
				textWrite(MyDc, "PLATELET");				
				textWrite(MyDc, "Fw =", stp->cplt.F2weak);
				textWrite(MyDc, "Fs =", stp->cplt.F2strong);
				textWrite(MyDc, "Ffib =", stp->cplt.F2fib);
				textWrite(MyDc, "time =", stp->cplt.F2t);
				textWrite(MyDc, "u crit =", stp->cplt.critFib);
				textWrite(MyDc, "dc =", stp->cplt.dc);
				textWrite(MyDc, "dd =", stp->cplt.dd);
				textWrite(MyDc, "plts =", stp->cplt.percent, "%");
				textNewRow();
			}
			...
			if(stp->fib.enabled)
			{
				...
				textWrite(MyDc, "alpha3 =", stp->fib.alpha3);
				textWrite(MyDc, "alpha4 =", stp->fib.alpha4);
				textWrite(MyDc, "k1 =", stp->fib.k1);
				textWrite(MyDc, "k3 =", stp->fib.k3);
				textWrite(MyDc, "gamma1 =", stp->fib.gamma1);
				textWrite(MyDc, "gamma2 =", stp->fib.gamma2);
				textWrite(MyDc, "gamma3 =", stp->fib.gamma3);
				textWrite(MyDc, "beta1 =", stp->fib.beta1);
				textWrite(MyDc, "beta2 =", stp->fib.beta2);
				textWrite(MyDc, "beta3 =", stp->fib.beta3);
